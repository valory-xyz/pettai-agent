...
const Icon = {
	Food: ({ className, width = 20, height = 18, ...props }) => (
		<svg className={className} xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 20 18" fill="none" {...props}>
	...
	Health: ({ className, width = 19, height = 18, ...props }) => (
		<svg className={className} xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 19 18" fill="none" {...props}>
	...
	Power: ({ className, width = 18, height = 22, ...props }) => (
		<svg className={className} xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 18 22" fill="none" {...props}>
	...
	Gamepad: ({ className, width = 23, height = 16, ...props }) => (
		<svg className={className} xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 23 16" fill="none" {...props}>
	...
	ChevronDown: ({ className, width = 16, height = 16, ...props }) => (
...
const STAT_SYMBOLS = {
	hunger: '\u{1F356}',    // meat
	health: '\u{2764}',     // heart
	energy: '\u{26A1}',     // lightning
	happiness: '\u{1F3AE}', // gamepad
	hygiene: '\u{1F6C1}',   // bathtub
};
...
.action-history-scroll::-webkit-scrollbar { width: 6px; }
.action-history-scroll::-webkit-scrollbar-track { background: transparent; }
.action-history-scroll::-webkit-scrollbar-thumb { background-color: rgba(255, 255, 255, 0.25); border-radius: 9999px; }
.action-history-scroll::-webkit-scrollbar-thumb:hover { background-color: rgba(255, 255, 255, 0.35); }
`;

const formatActionName = name =>
	name.replace(/_/g, ' ').toLowerCase().replace(/(^|\s)([a-z])/g, (_, p, c) => `${p}${c.toUpperCase()}`);
...
// ─── Inline stat bar (tiny horizontal bar for a 0-100 value) ────────────────
const StatBar = ({ label, value, color, symbol }) => {
	const pct = Math.max(0, Math.min(100, value ?? 0));
	const isLow = pct < 25;
	return (
		<div className="flex items-center gap-2 text-[10px]">
			<span className="w-4 text-center" title={label}>{symbol}</span>
			<div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
				<div
					className={clsx('h-full rounded-full transition-all', isLow && 'animate-pulse')}
					style={{ width: `${pct}%`, backgroundColor: color }}
				/>
			</div>
			<span className={clsx('w-7 text-right font-mono tabular-nums', isLow ? 'text-red-300' : 'text-white/60')}>
				{Math.round(pct)}
			</span>
		</div>
	);
...
// ─── Urgency dot (small colored indicator) ──────────────────────────────────
const UrgencyDot = ({ score }) => {
	if (score <= 0) return <span className="inline-block w-1.5 h-1.5 rounded-full bg-green-400/60" title="No urgency" />;
	if (score < 40) return <span className="inline-block w-1.5 h-1.5 rounded-full bg-yellow-400/70" title={`Urgency: ${score.toFixed(0)}`} />;
	return <span className="inline-block w-1.5 h-1.5 rounded-full bg-red-400/80 animate-pulse" title={`Urgency: ${score.toFixed(0)}`} />;
};
...
const AlternativesList = ({ alternatives }) => {
	...
	return (
		<div className="mt-2 space-y-0.5">
			<p className="text-[9px] uppercase tracking-widest text-white/40 mb-1">Candidates</p>
			{alternatives.slice(0, 5).map((alt, i) => (
				<div key={alt.action} className="flex items-center gap-1.5 text-[10px]">
					<span className={clsx(
						'w-1.5 h-1.5 rounded-full',
						i === 0 ? 'bg-green-400' : 'bg-white/20'
					)} />
					<span className={clsx('font-mono', i === 0 ? 'text-white/80' : 'text-white/40')}>
						{alt.action}
					</span>
					<span className="text-white/30 ml-auto font-mono tabular-nums">
						{alt.score > 0 ? `+${alt.score.toFixed(0)}` : alt.score.toFixed(0)}
					</span>
				</div>
			))}
		</div>
...
const ActionHistory = () => {
	...
	const handleInfoIconLeave = useCallback(() => {
		...
		closeTimerRef.current = setTimeout(() => { if (!bubbleHoverRef.current) setIsInfoOpen(false); }, 120);
	}, []);
	...
	const handleBubbleLeave = useCallback(() => { bubbleHoverRef.current = false; setIsInfoOpen(false); }, []);
	...
	const loadHistory = useCallback(async () => {
		try {
			...
			const next = {
				...
				last_decision_log: data?.last_decision_log ?? null,
			};
			setHistory(next);
	...
	useEffect(() => { if (styleRef.current) styleRef.current.textContent = SCROLLBAR_STYLES; }, []);
	...
	const entries = useMemo(() => {
		return [...history.actions]
			...
			.map((action, index) => {
				...
				const normalized = rawName.replace(/[^a-z0-9]+/gi, '_').replace(/_{2,}/g, '_').replace(/^_|_$/g, '').toUpperCase();
				const compactKey = normalized.replace(/_/g, '');
				const details = ACTION_DETAILS[normalized] || ACTION_DETAILS[compactKey] || {
	...
	return (
		...
		>
			<style ref={styleRef} />
			<div className="fixed inset-0" style={{ backgroundImage: `url(${backgroundOverlay})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.18, zIndex: 1 }} />
			<div className="fixed inset-0 bg-gradient-to-br from-purple-950/60 via-indigo-950/50 to-purple-950/60" style={{ zIndex: 2 }} />

			{/* Back button */}
			<button type="button" onClick={handleBack} className="fixed top-4 left-4 z-[99] text-white transition-colors bg-white/10 hover:bg-white/15 hover:text-white rounded-full p-2 shadow-lg shadow-purple-900/40 border border-white/20 backdrop-blur" aria-label="Back to dashboard" title="Back to dashboard">
			...
			<div className="flex-1 flex flex-col items-center relative px-4 pb-12 w-full" style={{ minHeight: '100vh', overflow: 'visible', paddingTop: '24px', zIndex: 10, paddingBottom: '48px' }}>
				<div className="flex flex-col items-center w-full max-w-[550px] mx-auto">
					...
					<div className="text-center text-white/90">
						...
						<p className="mt-2 text-white/70 text-sm font-medium">Current Run's Actions</p>
						{Number.isFinite(history.completed) && Number.isFinite(history.required_actions) ? (
							<p className="mt-1 text-white/60 text-xs uppercase tracking-widest">{history.completed} actions completed</p>
						) : null}
					...
					<div className="mt-6 w-full">
						<div className="bg-white/5 backdrop-blur-xl rounded-[32px] p-5 shadow-[0_18px_45px_rgba(2,6,23,0.45)] border border-white/10">
							<div className="flex items-center justify-between gap-3">
								...
								<div className="flex items-center gap-2">
									...
									<div className="relative" onMouseEnter={handleInfoOpen} onMouseLeave={handleInfoIconLeave}>
										<button type="button" className="inline-flex items-center justify-center size-9 rounded-full bg-white/8 border border-white/15 text-white/80 hover:text-white hover:bg-white/12 transition-colors" onClick={handleInfoOpen} aria-label="About Pett.AI connection">
											<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
												<path fill="currentColor" d="M10 18.333a8.333 8.333 0 1 0 0-16.666 8.333 8.333 0 0 0 0 16.666Zm-.833-11.25a.833.833 0 1 1 1.666 0a.833.833 0 0 1-1.666 0Zm0 2.5c0-.46.373-.833.833-.833c.46 0 .833.373.833.833V14.167a.833.833 0 1 1-1.666 0Z" />
											</svg>
										</button>
										<div onMouseEnter={handleBubbleEnter} onMouseLeave={handleBubbleLeave} className={clsx(
											...
											<p>
												...
												<a href="https://app.pett.ai" target="_blank" rel="noopener noreferrer" className="font-semibold text-purple-700 hover:text-purple-900">main Pett.AI app</a>
									...
									{/* Refresh */}
									<button type="button" onClick={loadHistory} className="text-white/80 text-xs font-medium bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 transition-all rounded-full px-4 py-1.5 shadow-inner shadow-black/20">
							...
							<div className="mt-5 relative">
								<div className="pointer-events-none absolute inset-x-0 top-0 h-20 backdrop-blur-xl bg-blue-900/20 z-20 [mask-image:linear-gradient(to_bottom,black,transparent)]" aria-hidden="true" />
								...
								<div className="action-history-scroll max-h-[60vh] overflow-y-auto overflow-x-hidden space-y-5 pr-1 pt-8 pb-12 relative z-10">
									{loading ? (
										<div className="flex items-center justify-center py-12 text-white/70 text-sm">Loading action history...</div>
									) : error ? (
										<div className="flex items-center justify-center py-12 text-red-200 text-sm">{error}</div>
									) : entries.length === 0 ? (
										<div className="flex items-center justify-center py-12 text-white/70 text-sm">No actions recorded yet today.</div>
									) : (
										entries.map((entry, index) => {
											...
											return (
												<div
													...
													className={clsx(
														'flex w-full',
														entry.isRightAligned ? 'justify-end' : 'justify-start',
														isLastEntry && 'mb-12',
														isFirstEntry && 'mt-8',
													)}
												>
													<div className={clsx('flex flex-col gap-2 max-w-[340px]', entry.isRightAligned ? 'items-end text-right' : 'items-start text-left')}>
														{/* Timestamp */}
														<span className="text-[11px] font-medium tracking-widest text-white/50 uppercase">{entry.time}</span>

														{/* Bubble */}
														<div className="relative w-full sm:max-w-[340px] rounded-3xl px-4 py-3 shadow-xl border border-white/10 bg-gradient-to-br from-purple-950/70 via-purple-900/60 to-indigo-950/60 text-white/90 backdrop-blur-2xl">
															{/* Accent stripe */}
															<span aria-hidden="true" className={clsx('absolute top-3 bottom-3 left-0 w-1 rounded-full', statStyle?.accent || 'bg-white/20')} />

															{/* Stat icon badge */}
															{entry.statKey ? (() => {
																const iconEl = renderStatIcon(entry.statKey);
																return iconEl ? (
																	<span className={clsx('absolute top-2 right-2 size-5 rounded-full flex items-center justify-center ring-1 ring-white/15 text-white', statStyle?.accent)} title={entry.stat}>
																		{iconEl}
																	</span>
																) : null;
															})() : null}
															...
															<div className="flex flex-col gap-2">
																...
																<div className="flex items-center gap-2 mr-6">
																	<p className="text-sm font-semibold tracking-tight text-white/90">{entry.title}</p>
																	{mode && (
																		<span className={clsx('inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider border', mode.bg, mode.text, mode.border)}>
																...
																{dl?.statsBefore && (
																	<div className="mt-1 space-y-0.5">
																		{['hunger', 'health', 'energy', 'happiness', 'hygiene'].map(stat => (
																			<StatBar
																				key={stat}
																				label={stat}
																				value={dl.statsBefore[stat]}
																				color={STAT_STYLES[stat]?.color || '#fff'}
																				symbol={STAT_SYMBOLS[stat]}
																			/>
																		))}
																	</div>
																)}
								...
								<div className="pointer-events-none absolute inset-x-0 bottom-0 h-20 backdrop-blur-xl bg-blue-900/20 z-20 [mask-image:linear-gradient(to_top,black,transparent)]" aria-hidden="true" />
							</div>
						</div>
					</div>
				</div>
